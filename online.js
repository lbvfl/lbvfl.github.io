(function () {
  ...
  var Defined = {
    api: 'lampac',
    localhost: 'http://192.168.1.11:9120/',
    apn: ''
  };
...
var hostkey = 'http://192.168.1.11:9120'.replace('http://', '').replace('https://', '');
...
window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
  if (!window.rch_nws[hostkey].startTypeInvoke) {
    ...
    if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
      ...
      net.silent('http://192.168.1.11:9120'.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
        check(true);
      }, function() {
        check(false);
      }, false, {
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  window.rch_nws[hostkey].typeInvoke('http://192.168.1.11:9120', function() {
    ...
    client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
	  ...
	  function sendResult(uri, html) {
	    $.ajax({
	      url: 'http://192.168.1.11:9120/rch/' + uri + '?id=' + rchId,
	      type: 'POST',
	      data: html,
	      async: true,
	      cache: false,
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["http://192.168.1.11:9120/js/nws-client-es5.js?v21042026"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      network.silent(
        account('http://192.168.1.11:9120/lite/withsearch'),
        function (json) {
  ...
  function startPlugin() {
    ...
    window.NextGen_plugin = true;
    Lampa.Component.add('NextGen', component);
    var manifest = {
      type: 'video',
    ...
    addSourceSearch('Lampac NextGen', 'spider');
    addSourceSearch('Lampac NextGen - Anime', 'spider/anime');
  ...
  if (!window.NextGen_plugin) startPlugin();
})();
